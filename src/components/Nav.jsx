...
const Nav = () => {
    ...
    const handleTogle = (e) => {
        if (e.target.checked) {
            setTheme('synthwave');
            // document.getElementById('navbar-div').classLinkst.add('bg-white');
        } else {
            setTheme('Linkght')
        }
    }
    return (
        <div id="navbar-div" className="navbar bg-base-100 shadow-xl px-6 sm:px-6 fixed z-10">
            <div className="flex-1">
                <Link to="/" className="btn btn-ghost text-xl text-secondary gap-0">Byte<span className="text-primary">Blaze</span></Link>
            </div>
